...
(defun bubble-buffer-ignore (str)
  (or
   ;;buffers I don't want to switch to
   (string-match "\\*Buffer List\\*" str)
   (string-match "\\*Bookmark List\\*" str)
...
(defun bubble-buffer-omit-buffer (buffer)
  ...
  (let ((buf-name (buffer-name buffer)))
    (unless (or (bubble-buffer-ignore buf-name) (and bubble-buffer-omit-regexp (string-match bubble-buffer-omit-regexp buf-name)))
        buf-name)))
